...
static void release_task(struct task_struct * p)
{
	...
	/* The incoming message queue and incoming messages will be deleted and their memory freed. */
	struct list_head *pos, *tmp;
	if (p->mpi_registered) {
		list_for_each_safe(pos, tmp, &p->message_queue) {
			list_del(pos);
			message_t *del_message = list_entry(pos, struct message, list);
			kfree(del_message->content);
			kfree(del_message);
		}
	}
