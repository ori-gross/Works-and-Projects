	    ...
	    struct pt_regs *regs, unsigned long stack_size)
{
	...
	*p = *current;
	...
	/* If the parent process is registered for MPI communication, the child process will also be
	   registered and have an empty queue. */
	if (current->mpi_registered) {
		INIT_LIST_HEAD(&p->message_queue);
		p->mpi_registered = 1;
		init_waitqueue_head(&p->mpi_wait_queue); // Initialize child's wait queue
	}
